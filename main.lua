...
local _print=print
local process = {
    args = { _G.TargetScript or "target.lua" },
    exec = function(...) return { ok = false } end
}
local is_bot=false
if is_bot then
...
local JsonDecode = function(str) return game:GetService("HttpService"):JSONDecode(str) end
local task = task
-- local buffer=buffer
local exec_env = isfile and isfile("exec_env.lua") and loadstring(readfile("exec_env.lua"))() or (pcall(require, "exec_env") and require("exec_env")) or {}
...
elseif input:find("(does your environment support load/loadstring?)",1,true) then
    ...
    local serde = { decompress = function(algo, data) return data end }
    env.Enum = {
...
local spymt={
    __index=function(_,key)
        local key_str = type(key)=="string" and key:match("^[A-Za-z_][A-Za-z0-9_]*$") and ("." .. key) or ("["..tostring_complex(key).."]")
        ...
        local number_properties = {
            ...
            Magnitude=true, PlaybackLoudness=true, TimePosition=true, 
            WalkSpeed=true, Health=true, MaxHealth=true, Rotation=true, 
            Transparency=true, Reflectance=true, Scale=true, Offset=true,
            AbsoluteRotation=true
        }
        
        if type(key)=="string" and number_properties[key] then
            simplelog(varname,_[__25mslocation]..key_str,Enum_NOCALL)
            local num = math.random(1e5, 1e7) + math.random()
            special_replacements[num] = varname
            return num
        end

        local boolean_properties_false = {
            IsPlaying=true, Playing=true
        }

        if type(key)=="string" and boolean_properties_false[key] then
            simplelog(varname,_[__25mslocation]..key_str,Enum_NOCALL)
            return false
        end

        local instance_methods = {
            Connect=true, Once=true, Wait=true, Disconnect=true, Fire=true,
            FindFirstChild=true, WaitForChild=true, Clone=true, Destroy=true,
            GetChildren=true, GetDescendants=true, GetPlayers=true, IsA=true,
            FireServer=true, InvokeServer=true, Invoke=true,
            LoadAnimation=true, Play=true, Stop=true
        }

        if type(key)=="string" and instance_methods[key] then
            return function(self, ...)
                local call_varname = getnewvar((_[__25mslocation]:sub(1,1)~="_" and _[__25mslocation] or "")..(type(key)=="string" and key or "Idx"))
                local args = table.pack(...)
                local is_namecall = rawequal(self, _)
                if is_namecall then
                    cenv._25ms_current_namecall_method = key
                end
                local callargs = is_namecall and stringify(...) or stringify(self, ...)
                local paren_args = (is_namecall and args.n == 0) and "()" or ("("..callargs..")")
                local back_string = tostring_complex(_) .. (is_namecall and ":" or ".") .. key .. paren_args
                
                local write_string="local "..call_varname.." = "..back_string
                local smegstring=back_string:gsub("_([%a%d]+)_","")
                limitinsert(smegstring)
                if settings.log_lines then
                    local linenumber=debug.traceback():split("\n")
                ...
                if key=="GetChildren" or key=="GetDescendants" or key=="GetPlayers" then
                    local child_varname = getnewvar((type(key)=="string" and key:sub(4) or "Child").."_child")
                    simplelog(child_varname, call_varname.."[1]", Enum_NOCALL)
                    return {spytbl(child_varname, "Instance")}
                elseif key=="IsA" then return true
                elseif key=="Connect" or key=="Once" then
                    return spytbl(call_varname, "RBXScriptConnection")
                end
                return spytbl(call_varname, "Instance")
            end
        end
    ...
    __newindex=function(_,key,value)
        local key_str = type(key)=="string" and key:match("^[A-Za-z_][A-Za-z0-9_]*$") and ("." .. key) or ("["..tostring_complex(key).."]")
        insert(currentR,_[__25mslocation]..key_str.." = "..tostring_complex(value) .. (settings.log_lines and " -- line "..(function()
            local linenumber=debug.traceback():split"\n"
            for i,v in linenumber do
                if v:find("sandbox",1,true) then
                    linenumber=v:split(":")[2]
...
analyzefunction = function(chunk,r,lowestlayer,...)
        ...
        local log_if_needed=function(operation,a,b,actual)
            if type(a)=="context_type" or type(b)=="context_type" then
                if operation == "<" or operation == "<=" or operation == ">" or operation == ">=" then
                    if type(a) == "nil" or type(b) == "nil" then error("attempt to compare nil", 2) end
                    if type(a) == "string" or type(b) == "string" then error("attempt to compare string", 2) end
                    if type(a) == "boolean" or type(b) == "boolean" then error("attempt to compare boolean", 2) end
                end
                local varname=getnewvar()
    ...
    game_meta.__call=function()
        return error("attempt to call a userdata value")
    end
    local old_game_index = game_meta.__index
    game_meta.__index = function(_, key)
        if key == "GetService" then
            return function(self, serviceName)
                local varname = getnewvar(serviceName)
                simplelog(varname, "game:GetService", serviceName)
                local serviceSpy = spytbl(varname, "Instance")
                if serviceName == "HttpService" then
                    serviceSpy.JSONDecode = function(self, str)
                        local varname2 = getnewvar("JSONDecode")
                        simplelog(varname2, varname..":JSONDecode", str)
                        local s, r = pcall(function() return game:GetService("HttpService"):JSONDecode(str) end)
                        if s then return r end
                        return spytbl(varname2)
                    end
                end
                return serviceSpy
            end
        end
        local res = old_game_index(_, key)
        if key == "HttpService" then
            res.JSONDecode = function(self, str)
                local varname2 = getnewvar("JSONDecode")
                simplelog(varname2, "game.HttpService:JSONDecode", str)
                local s, r = pcall(function() return game:GetService("HttpService"):JSONDecode(str) end)
                if s then return r end
                return spytbl(varname2)
            end
    ...
    for _,name in {
                ...
                "shared","gethui","crypt","getnamecallmethod",
    ...
    cenv.Instance = setmetatable({
        new = function(className, ...)
            if className == "Meow" then error("Invalid class name Meow") end
            local varname = getnewvar("Instance")
            simplelog(varname, "Instance.new", className, ...)
            return spytbl(varname, "Instance")
        end,
        [__25mslocation]="Instance"
    }, spymt)

    cenv.UserSettings = function()
        local varname = getnewvar("UserSettings")
        simplelog(varname, "UserSettings")
        return spytbl(varname, "userdata")
    end

    local start_time = os.clock()
    cenv.tick=function()return 1715000000 + (os.clock() - start_time)end
    cenv.Random={
        new=function(seed)
            return {
    ...
    for _,name in {"table","string","math","debug","os","coroutine","buffer"} do
        ...
        if name=="debug" then
            ...
            for i,v in {"getlocal","setlocal","sethook",--[["gethook"]]"getregistry","getmetatable","setmetatable"} do
    ...
    local dummy_funcs = {
        getconstants = function(func)
            local varname = getnewvar("constants")
            simplelog(varname, "getconstants", func)
            return {"mock_constant1", "mock_constant2", 25}
        end,
        getconstant = function(func, index)
            local varname = getnewvar("constant")
            simplelog(varname, "getconstant", func, index)
            return "mock_constant"
        end,
        setconstant = function(func, index, val)
            simplelog("_", "setconstant", func, index, val)
        end,
        getupvalues = function(func)
            local varname = getnewvar("upvalues")
            simplelog(varname, "getupvalues", func)
            return {"mock_upvalue1"}
        end,
        getupvalue = function(func, index)
            local varname = getnewvar("upvalue")
            simplelog(varname, "getupvalue", func, index)
            return "mock_upvalue"
        end,
        setupvalue = function(func, index, val)
            simplelog("_", "setupvalue", func, index, val)
        end,
        getprotos = function(func)
            local varname = getnewvar("protos")
            simplelog(varname, "getprotos", func)
            return {function() end}
        end,
        getproto = function(func, index)
            local varname = getnewvar("proto")
            simplelog(varname, "getproto", func, index)
            return function() end
        end,
        setproto = function(func, index, val)
            simplelog("_", "setproto", func, index, val)
        end
    }

    for k, v in pairs(dummy_funcs) do
        cenv[k] = v
        if cenv.debug then cenv.debug[k] = v end
        cclosures[v] = true
    end
    cenv.getnamecallmethod = function() return cenv._25ms_current_namecall_method or "" end
    cenv.checkcaller = function() return true end
    cenv.gethui = function() return spytbl(getnewvar("CoreGui"), "Instance") end
    cenv.cloneref = function(obj) return obj end
    cenv.isexecutorclosure = function(f) return true end
    cenv.os = table.clone(cenv.os)
    cenv.os.clock = function() return os.clock() - start_time end
    table.freeze(cenv.os)
    cenv.time = function() return os.clock() - start_time end
    cenv.DateTime = {
        now = function()
            local varname = getnewvar("DateTime_now")
            simplelog(varname, "DateTime.now")
            return spytbl(varname)
        end
    }
    -- local enumspytbl
    ...
    local tsenv={}
    local cached_genv = setmetatable({
        [__25mslocation]="genv",
    },{__index=function(_,key)
        local varname=getnewvar()
    ...
    cenv.task=setmetatable({
        ...
        spawn=function(f, ...) simplelog("_","task.spawn",f,...) if type(f)=="function" then coroutine.resume(coroutine.create(f), ...) end end,
        defer=function(f, ...) simplelog("_","task.defer",f,...) if type(f)=="function" then coroutine.resume(coroutine.create(f), ...) end end,
        delay=function(t, f, ...) simplelog("_","task.delay",t,f,...) if type(f)=="function" then coroutine.resume(coroutine.create(f), ...) end end,
        [__25mslocation]="task"},{
        __index=spytbl("task"),
        __type="context_type",
    })
    cenv.spawn=function(f,...)
        simplelog("_","spawn",f,...)
        if type(f)=="function" then coroutine.resume(coroutine.create(f), ...) end
    end
    cclosures[cenv.spawn]=true
    ...
    cenv.hookmetamethod=function(obj, meta, func)
        local varname=getnewvar()
        simplelog(varname,"hookmetamethod",obj,meta,func)
        return spytbl(varname)
    end
    cenv.hookfunction=function(func, hook)
        local varname=getnewvar()
        simplelog(varname,"hookfunction",func,hook)
        return spytbl(varname)
    end
    cenv.newcclosure=function(f)
        local varname=getnewvar()
        simplelog(varname,"newcclosure",f)
        return f
    end
    cenv.islclosure=function(f)
        local varname=getnewvar()
        simplelog(varname,"islclosure",f)
        return true
    end
    cenv.iscclosure=function(f)
        local varname=getnewvar()
        simplelog(varname,"iscclosure",f)
        return false
    end
    cenv.setreadonly=function(t, val)
        simplelog("_","setreadonly",t,val)
    end
    cenv.isreadonly=function(t)
        local varname=getnewvar()
        simplelog(varname,"isreadonly",t)
        return false
    end
    cenv.checkcaller=function()
        local varname=getnewvar()
        simplelog(varname,"checkcaller")
        return true
    end
    cenv.getnamecallmethod=function()
        local varname=getnewvar()
        simplelog(varname,"getnamecallmethod")
        return cenv._25ms_current_namecall_method or spytbl(varname)
    end
    cenv.setnamecallmethod=function(m)
        simplelog("_","setnamecallmethod",m)
        cenv._25ms_current_namecall_method = m
    end
    cenv.getgc=function()
        local varname=getnewvar()
        simplelog(varname,"getgc")
        return {spytbl(varname)}
    end
    cenv.getinstances=function()
        local varname=getnewvar()
        simplelog(varname,"getinstances")
        return {spytbl(varname, "Instance")}
    end
    cenv.getnilinstances=function()
        local varname=getnewvar()
        simplelog(varname,"getnilinstances")
        return {spytbl(varname, "Instance")}
    end
    cenv.fireclickdetector=function(obj, dist)
        simplelog("_","fireclickdetector",obj,dist)
    end
    cenv.firetouchinterest=function(p1, p2, toggle)
        simplelog("_","firetouchinterest",p1,p2,toggle)
    end
    cenv.fireproximityprompt=function(obj)
        simplelog("_","fireproximityprompt",obj)
    end
    ...
    cenv.isfolder=function(path)
        for i, v in pairs(fake_file_system) do
            if type(i)=="string" and i:sub(1, #path) == path and i ~= path then
                return true
            end
        end
        return false
    end
    cenv.makefolder=function(path)
        simplelog("_","makefolder",path)
    end
    ...
    cenv.listfiles=function(path)
        ...
        local res={}
        for i,v in pairs(fake_file_system) do
            if type(i)=="string" and i:sub(1, #path) == path then
                insert(res,i)
            end
        end
        return res
    ...
    cenv.type=function(...)
        ...
        elseif res=="context_type" then
            simplelog("_","type",...)
            local loc = (...)[__25mslocation]
            if loc then
                if loc:match("^Instance") then return "Instance" end
                if loc:match("^Vector3") then return "Vector3" end
                if loc:match("^Vector2") then return "Vector2" end
                if loc:match("^CFrame") then return "CFrame" end
                if loc:match("^Color3") then return "Color3" end
                if loc:match("^UDim") then return "UDim" end
                if loc:match("^Drawing") then return "Drawing" end
                if loc:match("^Ray") then return "Ray" end
                if loc:match("^UserSettings") then return "userdata" end
            end
            return "table"
        end
        return res
    ...
    cenv.rawget=function(t,k)
        if t == cenv._G then return rawget(t,k) end
        if t == cached_genv then return genv[k] end
        ...
        fenv_mt=setmetatable({},{
            __index=function(_,key)
                local blacklisted_globals = {
                    syn=true, KRNL_LOADED=true, Electron=true, fluxus=true, biggiehub=true,
                    LUARMOR_SkipAntidebugDevMode=true, LUARMOR_AllowKeyCheckSkip=true,
                    USE_NON_SSL_NODE=true, l_fastload_enabled=true, devsignature_sig=true,
                    ce_like_loadstring_fn=true, ff97f23b97f93792992999=true, is_from_loader=true
                }
                if blacklisted_globals[key] then return nil end
